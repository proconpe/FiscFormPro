...
import { AutoDeConstataçãoSchema, AutoDeNotificaçãoShema, RelatorioVisitaSchema } from "@/lib/schemas";
import { z } from "zod";
...
export default function NotificacaoForm() {
  ...
  const form = useForm<z.infer<typeof AutoDeConstataçãoSchema>>({
    resolver: zodResolver(AutoDeConstataçãoSchema),
  });

  useBuscarCep(form, startTransition);
  // Modificar o handleSubmit para redirecionar para a página de assinatura
  const onSubmit = (data: z.infer<typeof AutoDeConstataçãoSchema>) => {
    console.log(data);
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <Card className="max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>Constatação</CardTitle>
          <CardDescription>
            Preencha os dados para emitir o auto de constatação.
          </CardDescription>
        </CardHeader>
        <Form {...form}>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Dispositivos Legais Infrigados</FormLabel>
                      <FormControl>
                        <Input {...field} />
                      </FormControl>
