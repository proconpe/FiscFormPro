...
export default function NotificacaoSignature() {
  ...
  const formData = {
    ...
    tipoNotificacao: searchParams.get("tipoNotificacao") || "Solicitação de Documentos",
    motivoNotificacao: searchParams.get("motivoNotificacao") || "Motivo detalhado da notificação...",
    itensSolicitados: ["Contrato Social", "CNPJ", "Alvará de Funcionamento"],
    outrosItens: searchParams.get("outrosItens") || "Outros itens solicitados...",
    prazo: searchParams.get("prazo") || "10",
  }
  ...
  // Array de fiscais
  const [fiscais, setFiscais] = useState<Fiscal[]>([{ nome: "", matricula: "", assinatura: "" }])
  ...
  const atualizarFiscal = (index: number, campo: keyof Fiscal, valor: string) => {
    const novosFiscais = [...fiscais]
  ...
  const handleBack = () => {
    if (step > 1) {
      ...
      router.push("/forms/notificacao")
    }
  }

  // Verificar se todos os fiscais têm os dados preenchidos
  const fiscaisCompletos = fiscais.every(
    (fiscal) => fiscal.nome.trim() !== "" && fiscal.matricula.trim() !== "" && fiscal.assinatura !== "",
  )
  ...
  return (
    <div className="container mx-auto py-6">
      <Button variant="ghost" className="mb-4 flex items-center gap-1" onClick={handleBack}>
      ...
      {step === 1 && (
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            ...
            <CardDescription>
              Por favor, preencha os dados e assine o documento para finalizar a Notificação.
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button
              onClick={handleNext}
              disabled={!responsavelNome || !responsavelCargo || !responsavelSignature}
      ...
      {step === 2 && (
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            ...
            <CardDescription>
              Por favor, preencha os dados e assinaturas de todos os fiscais responsáveis.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {fiscais.map((fiscal, index) => (
              <div key={index} className="space-y-4 border p-4 rounded-md">
                <div className="flex justify-between items-center">
                  ...
                  {fiscais.length > 1 && (
                    <Button variant="ghost" size="sm" onClick={() => removerFiscal(index)} className="h-8 w-8 p-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor={`fiscal-nome-${index}`}>Nome do Fiscal</Label>
                    <Input
                      id={`fiscal-nome-${index}`}
                      value={fiscal.nome}
                      onChange={(e) => atualizarFiscal(index, "nome", e.target.value)}
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor={`fiscal-matricula-${index}`}>Matrícula</Label>
                    <Input
                      id={`fiscal-matricula-${index}`}
                      value={fiscal.matricula}
                      onChange={(e) => atualizarFiscal(index, "matricula", e.target.value)}
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <SignaturePad onSave={(dataUrl) => atualizarFiscal(index, "assinatura", dataUrl)} />
                </div>
              </div>
            ))}
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button onClick={handleNext} disabled={!fiscaisCompletos} className="flex items-center gap-1">
