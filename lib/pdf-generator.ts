...
export const generatePDF = async (data:RelatorioDeVisita) => {
  ...
  const maxContentHeight = pageHeight - margin - headerHeight - margin - footerHeight
  ...
  const addHeader = () => {
    ...
    pdf.text("GOVERNO DO ESTADO DE PERNAMBUCO", centerX, margin + 5, { align: "center" })

    pdf.setFont("helvetica", "normal")
    pdf.setFontSize(6)
    pdf.text("SECRETARIA DE DESENVOLVIMENTO SOCIAL E DIREITOS HUMANOS", centerX, margin + 9, { align: "center" })
    pdf.text("SECRETARIA EXECUTIVA DE JUSTIÇA E DIREITOS HUMANOS", centerX, margin + 13, { align: "center" })
    pdf.text("COORDENADORIA GERAL DE PROTEÇÃO E DEFESA DO CONSUMIDOR - PROCON", centerX, margin + 17, {
      align: "center",
    })
    ...
    pdf.setFontSize(10)
    pdf.setFont("helvetica", "bold")
    pdf.text("RELATÓRIO", pageWidth - margin - 25, margin + 7, { align: "center" })
    pdf.text("DE VISITA", pageWidth - margin - 25, margin + 12, { align: "center" })
  ...
  const availableSpaceForOcorrencias = pageHeight - margin - footerHeight - yPosition

  // Preparar o texto das ocorrências
  pdf.setFontSize(9)
  const ocorrenciasText = data.ocorrencias || ""
  const ocorrenciasLines = pdf.splitTextToSize(ocorrenciasText, contentWidth - 10)
  const lineHeight = 5
  ...
  if (ocorrenciasHeight > availableSpaceForOcorrencias - 10 && availableSpaceForOcorrencias < 100) {
    addFooter()
    pdf.addPage()
    currentPage++
  ...
  if (ocorrenciasLines.length > maxLines) {
    ...
    const continuationHeight = Math.min(remainingTextHeight + 20, maxContentHeight - 10)
  ...
  addSplitField([
    ...
    { title: "HORA", value: new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) },
  ])
  ...
  pdf.setFontSize(9)
  pdf.setFont("helvetica", "bold")
  pdf.text("4. FISCALIZADO", margin + 2, yPosition + 5)
  ...
  if (data.responsavelSignature) {
    try {
      pdf.addImage(data.responsavelSignature, "PNG", margin + 70, yPosition + 10, 60, 15)
    } catch (error) {
  ...
  if (numFiscaisLimitados > 0) {
    ...
    for (let i = 0; i < numFiscaisLimitados; i++) {
      ...
      if (fiscal.assinatura) {
        try {
          pdf.addImage(fiscal.assinatura, "PNG", xPos + 35, yPosition + 5, 50, 15)
        } catch (error) {
          console.error(`Erro ao adicionar assinatura do fiscal ${i + 1}:`, error)
        }
      }

      // Adicionar linha para assinatura
      pdf.line(xPos + 35, yPosition + 25, xPos + 85, yPosition + 25)
      pdf.setFontSize(8)
      pdf.text("ASSINATURA", xPos + 60 - 15, yPosition + 30)
    }
  ...
  // Salvar o PDF
  pdf.save(`Relatório_de_Visita_${data.razaoSocial.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`)
}
